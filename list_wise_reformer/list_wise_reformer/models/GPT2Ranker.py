...
class GPT2Ranker():
    ...
    def __init__(self, args, sacred_ex):
        ...
        self.tokenizer = GPT2Tokenizer. \
            from_pretrained(args.gpt2_model)
        self.model = GPT2Model.from_pretrained(args.gpt2_model)
    ...
    def fit(self, sessions, validation_sessions):
        ...
        if self.args.multi_task_for != "":
            train_file_path = self.args.data_folder + self.args.task + "/train_examples_gpt2_{}.pk".format(
                self.args.multi_task_for)
        else:
            train_file_path = self.args.data_folder + self.args.task + "/train_examples_gpt2.pk"
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.logging_steps == 0:
                    ...
                    logging.info("GPT2 validation scores")
    ...
    def predict(self, sessions, prediction_cols, training_eval=False, adv_eval=False):
        examples = self.processor. \
            get_examples_from_sessions(sessions,
                                       len(prediction_cols))
        if training_eval:
            path = self.args.data_folder + self.args.task + "/val_for_train_examples_gpt2.pk"
        elif adv_eval:
            path = self.args.data_folder + self.args.task + "/adv_test_examples_gpt2.pk"
        else:
            path = self.args.data_folder + self.args.task + "/test_examples_gpt2.pk"
