...
class DialoRanker():
    ...
    def fit(self, sessions, validation_sessions):
        ...
        if self.args.multi_task_for != "":
            train_file_path = self.args.data_folder + self.args.task + "/train_examples_dialo_{}.pk".format(
                self.args.multi_task_for)
        else:
            train_file_path = self.args.data_folder + self.args.task + "/train_examples_dialo.pk"
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.logging_steps == 0:
                    ...
                    results = {'dialo_val': {}}
                    results['dialo_val']['preds'] = preds
                    # only first doc is relevant -> [1, 0, 0, ..., 0]
                    labels = [[1] + ([0] * (len(validation_sessions.columns[1:])))
                              for _ in range(validation_sessions.shape[0])]
                    results['dialo_val']['labels'] = labels
                    results = evaluate_models(results)
                    logging.info("Dialo validation scores")
                    for metric in ['recip_rank', 'ndcg_cut_10']:
                        res = 0
                        for q in results['dialo_val']['eval'].keys():
                            res += results['dialo_val']['eval'][q][metric]
                        res /= len(results['dialo_val']['eval'].keys())
    ...
    def predict(self, sessions, prediction_cols, training_eval=False, adv_eval=False):
        examples = self.processor. \
            get_examples_from_sessions(sessions,
                                       len(prediction_cols))
        if training_eval:
            path = self.args.data_folder + self.args.task + "/val_for_train_examples_dialo.pk"
        elif adv_eval:
            path = self.args.data_folder + self.args.task + "/adv_test_examples_dialo.pk"
        else:
            path = self.args.data_folder + self.args.task + "/test_examples_dialo.pk"
